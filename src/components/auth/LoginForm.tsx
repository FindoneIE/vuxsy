...
// LoginForm receives `redirect` as a prop instead of calling
// useSearchParams(). This removes the need for a wrapping <Suspense>
// fallback on the /login route — the page can SSR its full tree in one
// render with no CSR bailout, eliminating the white-flash + footer jump
// that previously occurred on every login navigation.
export default function LoginForm({ redirect }: { redirect: string }) {
  const router = useRouter();
  const safeRedirect = redirect.startsWith("/") ? redirect : "/dashboard/listings";
