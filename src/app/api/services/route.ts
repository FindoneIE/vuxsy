...
export async function GET(req: Request) {
  try {
    ...
    console.log("API SERVICES QUERY:", { category, county, area, pageSize });

    const result = await getListings({ type: "service", category, county, area, pageSize });

    const items = (result.items || []).map((it) => serializeListing(it));

    console.log("API SERVICES RESULT:", { count: items.length });
...
export async function POST() {
  return new Response("create service");
}
